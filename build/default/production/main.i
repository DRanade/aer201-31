...
void showTime(unsigned char pTime[7]){
    ...
    for(unsigned char i = 0; i < 6; i++){
        pTime[i] = I2C_Master_Read(0);
    }
    pTime[6] = I2C_Master_Read(1);
    I2C_Master_Stop();
...
void scrRunning(unsigned char time[3]){
    unsigned int i;
    { lcdInst(0x80); _delay((unsigned long)((2)*(10000000/4000.0)));};
...
void scrDonePoleDet(poleInfo poleData[15],int currPole){
    { lcdInst(0x80); _delay((unsigned long)((2)*(10000000/4000.0)));};
    printf("Pole%01x Pos:%01d.%03d",
    poleData[currPole].id,poleData[currPole].pos/1000,poleData[currPole].pos % 1000);
    { lcdInst(0x80 | LCD_LINE2_ADDR);};
...
poleCount = 4;
    while(1){
        ...
        if (dispMode == 0){
            ...
            if (currPole < poleCount){ currPole += 1; }
            dispMode = 4;
            scrDonePoleDet (poleInfoArr,currPole);
        }
